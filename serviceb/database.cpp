...
void Database::requestUpdateEmployeeInfo(const EmployeeInfo &emplinfo){
    if(m_employeeList.size() == 0){
        return;
    }
    for(auto& e : m_employeeList){
        if(e.id == emplinfo.id){
            strncpy(e.name , emplinfo.name, MAX_NAME_CHAR);
            e.asm_score = emplinfo.asm_score;
            e.cpp_score = emplinfo.cpp_score;
            e.js_score = emplinfo.js_score;
            e.opengl_score = emplinfo.opengl_score;
            e.qml_score = emplinfo.qml_score;
        }
    }
    // set to shm, but this is inf file
    json j_obj = json::array();
